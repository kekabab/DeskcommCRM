...
export type ChannelSessionRef =
  ...
  | { provider: "simulator"; simulator_session_key: string };
...
export const CHANNEL_SESSION_REF_COLUMNS =
  "provider, waha_session_name, meta_phone_number_id, zernio_account_id, simulator_session_key";
...
export function resolveSessionRef(session: ChannelSessionRef): string {
  switch (session.provider) {
    ...
    case "simulator":
      return session.simulator_session_key;
  }
}
